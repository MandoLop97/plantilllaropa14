...
const Citas = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex flex-col">
      ...
      <main className="flex-grow py-4 sm:py-8 lg:py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex justify-center">
              <div className="flex items-center space-x-2 sm:space-x-4 lg:space-x-8 overflow-x-auto pb-2">
                ...
                <div className={`flex items-center ${getStepStatus('servicio') === 'completed' ? 'text-green-600' : getStepStatus('servicio') === 'active' ? 'text-blue-600' : 'text-gray-400'}`}>
                  ...
                  <span className="font-medium text-sm sm:text-base hidden sm:inline">Servicio</span>
                  <span className="font-medium text-xs sm:hidden">1</span>
                </div>
                
                {/* Línea de conexión */}
                <div className={`w-4 sm:w-8 lg:w-12 h-0.5 ${getStepStatus('servicio') === 'completed' ? 'bg-green-200' : 'bg-gray-200'}`}></div>
                ...
                <div className={`flex items-center ${getStepStatus('calendario') === 'completed' ? 'text-green-600' : getStepStatus('calendario') === 'active' ? 'text-blue-600' : 'text-gray-400'}`}>
                  ...
                  <span className="font-medium text-sm sm:text-base hidden sm:inline">Fecha y Hora</span>
                  <span className="font-medium text-xs sm:hidden">2</span>
                </div>
                
                {/* Línea de conexión */}
                <div className={`w-4 sm:w-8 lg:w-12 h-0.5 ${getStepStatus('calendario') === 'completed' ? 'bg-green-200' : 'bg-gray-200'}`}></div>
                ...
                <div className={`flex items-center ${getStepStatus('datos') === 'active' ? 'text-blue-600' : 'text-gray-400'}`}>
                  ...
                  <span className="font-medium text-sm sm:text-base hidden sm:inline">Confirmar</span>
          ...
          <div className="max-w-6xl mx-auto">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className={`grid w-full grid-cols-3 mb-6 sm:mb-8 bg-white/80 backdrop-blur-sm shadow-sm border ${isMobile ? 'h-12' : 'h-14'}`}>
                <TabsTrigger 
                  value="servicios" 
                  className={`flex items-center justify-center ${isMobile ? 'text-xs px-2' : 'text-sm px-4'}`}
                >
              ...
              <TabsContent value="servicios" className="space-y-4 sm:space-y-6">
                      ...
                      <div className="mt-6 p-4 bg-gradient-to-r from-green-50 to-blue-50 rounded-xl border border-green-200">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            ...
                            <span className="text-green-700 font-medium">Servicio seleccionado</span>
                          </div>
                          <button
              ...
              <TabsContent value="calendario" className="space-y-4 sm:space-y-6">
                <Card className="border-0 shadow-xl bg-white/80 backdrop-blur-sm">
                  <CardHeader className="pb-4 sm:pb-6">
                    <CardTitle className="flex items-center text-xl sm:text-2xl">
                    ...
                    {fechaSeleccionada && horaSeleccionada && (
                      <div className="mt-6 p-4 bg-gradient-to-r from-green-50 to-blue-50 rounded-xl border border-green-200">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            <CheckCircle className="w-5 h-5 text-green-600 mr-2" />
                            <span className="text-green-700 font-medium">Fecha y hora seleccionadas</span>
                          </div>
                          <button
              ...
              <TabsContent value="datos" className="space-y-4 sm:space-y-6">
                <CitasForm 
