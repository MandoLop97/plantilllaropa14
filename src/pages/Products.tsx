...
const Products = () => {
  ...
  const { products, isLoading } = useEnhancedSupabaseProducts(undefined, BUSINESS_ID);
            ...
            <div className="max-w-3xl mx-auto w-full">
              <div className="flex flex-wrap items-center justify-center gap-2 sm:justify-between sm:flex-nowrap bg-white/80 rounded-xl px-2 py-3 shadow-sm">
                <div className="flex items-center gap-2 flex-wrap justify-center">
                  ...
                  {filters.category && (
                    <span className="bg-picton-blue-100 text-picton-blue-600 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1">
                      {categories.find(cat => cat.name === filters.category)?.icon}
                      {categories.find(cat => cat.name === filters.category)?.name}
                      <button
                        onClick={() => handleFilterChange({ category: '' })}
                        className="ml-1 text-picton-blue-500 hover:text-red-500 focus:outline-none"
                ...
                <div className="flex items-center gap-2 flex-wrap justify-center">
                  ...
                  <motion.button
                    ...
                    className={`lg:hidden flex items-center gap-2 px-4 py-2 rounded-full border transition-colors shadow-sm ${
                      showFilters ? 'bg-picton-blue-500 text-white border-picton-blue-500' : 'bg-white text-gray-700 border-gray-300 hover:border-picton-blue-500'
                    }`}
                  >
                    <Filter size={16} />
                  ...
                  <div className="flex border border-gray-300 rounded-full overflow-hidden bg-gray-50">
                    <button
                      ...
                      className={`p-2 transition-colors ${viewMode === 'grid' ? 'bg-picton-blue-500 text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                    >
                      <Grid size={16} />
                    </button>
                    <button
                      onClick={() => setViewMode('list')}
                      className={`p-2 transition-colors ${viewMode === 'list' ? 'bg-picton-blue-500 text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                    >
                      <List size={16} />
                    </button>
                  </div>
                  ...
                  {hasActiveFilters && (
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-picton-blue-100 text-picton-blue-600 hover:bg-picton-blue-200 hover:text-picton-blue-800 font-semibold text-xs shadow transition-all border border-picton-blue-200"
