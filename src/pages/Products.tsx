...
const Products = () => {
  ...
  const { products, loading } = useEnhancedSupabaseProducts(businessId || '');
  ...
  return (
    <div className="min-h-screen ice-cream-pattern flex flex-col">
            ...
            <div className="max-w-3xl mx-auto w-full">
              <div className="flex flex-wrap items-center justify-center gap-2 sm:justify-between sm:flex-nowrap bg-transparent-glass rounded-xl px-2 py-2 shadow-sm">
                <div className="flex items-center gap-2 flex-wrap justify-center">
                  ...
                  {filters.category && (
                    <span className="bg-primary-100 text-primary-600 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1">
                      {categories.find(cat => cat.id === filters.category)?.icon}
                      {categories.find(cat => cat.id === filters.category)?.name}
                      <button
                        onClick={() => handleFilterChange({ category: '' })}
                        className="ml-1 text-primary-500 hover:text-red-500 focus:outline-none"
                ...
                <div className="flex items-center gap-2 flex-wrap justify-center">
                  ...
                  <motion.button
                    ...
                    className={`lg:hidden flex items-center gap-2 px-3 py-1.5 rounded-full border transition-colors shadow-sm ${
                      showFilters ? 'bg-primary-500 text-white border-primary-500' : 'bg-white text-neutral-700 border-neutral-300 hover:border-primary-500'
                    }`}
                  >
                    <Filter size={14} />
                  ...
                  <div className="flex border border-neutral-300 rounded-full overflow-hidden bg-neutral-50">
                    <button
                      ...
                      className={`p-1.5 transition-colors ${viewMode === 'grid' ? 'bg-primary-500 text-white' : 'text-neutral-600 hover:bg-neutral-100'}`}
                    >
                      <Grid size={14} />
                    </button>
                    <button
                      onClick={() => setViewMode('list')}
                      className={`p-1.5 transition-colors ${viewMode === 'list' ? 'bg-primary-500 text-white' : 'text-neutral-600 hover:bg-neutral-100'}`}
                    >
                      <List size={14} />
                    </button>
                  </div>
      ...
      <a
        ...
        className={`fixed bottom-4 left-4 bg-green-500 text-white rounded-full p-2.5 shadow-lg hover:bg-green-600 ${UI_CONFIG.TRANSITION_CLASSES} z-40 flex items-center justify-center`}
