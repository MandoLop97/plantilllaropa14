...
export const DataSyncPanel: React.FC = () => {
  ...
  const handleSyncProducts = async () => {
    setLoading(true);
    try {
      const stats = await DataSync.syncProductsToSupabase();
      setSyncStats(stats);
  ...
  const handleSyncCategories = async () => {
    setLoading(true);
    try {
      const stats = await DataSync.syncCategoriesToSupabase();
      setSyncStats(stats);
  ...
  const handleExportData = async () => {
    setLoading(true);
    try {
      const result = await DataSync.exportData();
      if (result) {
  ...
  const handleClearData = async () => {
    ...
    setLoading(true);
    try {
      const success = await DataSync.clearSupabaseData();
      if (success) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 max-w-2xl mx-auto">
      <h3 className="text-xl font-bold text-gray-900 mb-6">Panel de Sincronización de Datos</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center justify-center gap-2 px-4 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600 disabled:opacity-50 transition-colors"
        >
          {loading ? <RefreshCw className="animate-spin" size={16} /> : <Upload size={16} />}
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center justify-center gap-2 px-4 py-3 bg-purple-500 text-white rounded-lg hover:bg-purple-600 disabled:opacity-50 transition-colors"
        >
