  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto mb-8 rounded-2xl overflow-hidden shadow-lg">
      ...
      <motion.button
        ...
        onClick={prev}
        className="absolute left-2 top-1/2 -translate-y-1/2 bg-white hover:bg-gray-50 rounded-full p-2 shadow-md"
        aria-label="Anterior"
      >
        <ChevronLeft className="w-6 h-6 text-picton-blue-500" />
      </motion.button>
      ...
      <motion.button
        ...
        whileHover={{ scale: 1.08 }}
        onClick={next}
        className="absolute right-2 top-1/2 -translate-y-1/2 bg-white hover:bg-gray-50 rounded-full p-2 shadow-md"
        aria-label="Siguiente"
      >
        <ChevronRight className="w-6 h-6 text-picton-blue-500" />
      </motion.button>
        ...
        {bannerImages.map((_, idx) => (
          <span
            ...
            className={`w-2 h-2 rounded-full transition-colors duration-300 ${
              idx === current ? 'bg-picton-blue-500' : 'bg-gray-300'
            }`}
          />
        ))}
      </div>
