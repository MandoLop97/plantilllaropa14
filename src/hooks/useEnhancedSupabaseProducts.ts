...
export const useEnhancedSupabaseProducts = (category?: string, businessId?: string) => {
  const queryClient = useQueryClient();
  const { toast } = useToast();

  // Query para obtener productos
  const productsQuery = useQuery({
    queryKey: category ? [...QUERY_KEYS.productsByCategory(category), businessId] : [...QUERY_KEYS.products, businessId],
    queryFn: async () => {
